...
class VideoToolGUI(QMainWindow):
    ...
    def select(self):
        ...
        video_file = QFileDialog.getOpenFileName(self, "Open Video", self.init_open_dir(), "Video Files (*.avi *.mp4 *.ogv)")[0]
    ...
    def read_video_info(self, video_file):
        ...
        if cap.isOpened():
            ...
            self.fps = 30#cap.get(cv2.CAP_PROP_FPS)
    ...
    def grab_img(self):
        ...
        if save_dir:
            ...
            progress = QProgressDialog(self)
            progress.setWindowTitle("Image Save")  
            progress.setLabelText("Process video and saving...                ")
            progress.setMinimumDuration(1)
            progress.setWindowModality(Qt.WindowModal)
            progress.setFixedWidth(600)
            ...
            if rect != img_rect:
                save_rect = tuple([int(x*resize_rate) for x in rect])
            save_cnt = 0
            while ret and (end_idx<0 or frame_idx<=end_idx):
